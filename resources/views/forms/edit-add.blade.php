...
@section('page_title', __('versatile::generic.'.(isset($form->id) ? 'edit' : 'add')).' '.$dataType->display_name_singular)
...
@section('page_header')
    <h1 class="page-title">
        ...
        {{ __('versatile::generic.'.(isset($form->id) ? 'edit' : 'add')).' '.$dataType->display_name_singular }}
    </h1>
@stop
        ...
        <div class="row">
            <div class="col-md-4">
                <div class="panel">
                    ...
                    <div class="panel-heading">
                        <h3 class="panel-title"><i class="versatile-info-circled"></i> Form Details</h3>
                    </div> <!-- /.panel-heading -->
                    ...
                    <div class="panel-body">
                        <form
                            ...
                            {{ route('versatile.'.$dataType->slug.'.update', $form->id) }}
                            @else
                            {{ route('versatile.'.$dataType->slug.'.store') }}
                            @endif"
                            method="POST"
                            ...
                            <button type="submit" class="btn btn-primary">
                                {{ __('versatile::generic.'.(isset($form->id) ? 'update' : 'add')) }}
                ...
                @if (isset($form))
                    <div class="panel panel-bordered panel-warning">
                        <div class="panel-heading">
                            <h3 class="panel-title"><i class="versatile-plus"></i> Add Field</h3>
                        </div> <!-- /.panel-heading -->

                        <div class="panel-body">
                            <form role="form" action="{{ route('versatile.inputs.store') }}" method="POST"
                                enctype="multipart/form-data">
                                {{ csrf_field() }}
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="type" id="type">
                                        ...
                                        @foreach (config('versatile-forms.available_inputs') as $key => $value)
                                ...
                                <button type="submit"
                                        class="btn btn-success btn-sm">{{ __('versatile::generic.add') }}</button>
                            </form>
                        </div> <!-- /.panel-body -->
                    </div> <!-- /.panel -->
                @endif
            ...
            <div class="col-md-8">
                @if (isset($form))
                    <div class="dd">
                        <ol class="dd-list">
                            @each('versatile-forms::inputs.edit-add', $form->inputs, 'input')
                        </ol>
                    </div> <!-- /.dd -->
                @endif
            </div>
